...
# --- Indexing Logic (Modified to handle existing index conflict) ---
try:
    # Attempt to drop the conflicting index first
    # This tries to remove the 'message_id_1' index if it exists and causes conflict
    # The name 'message_id_1' is auto-generated by PyMongo for an index on 'message_id' field.
    movies_col.drop_index("message_id_1")
    print("Existing 'message_id_1' index dropped successfully (if it existed).")
except Exception as e:
    # If the index doesn't exist, drop_index will raise an error.
    # We can ignore it if the error indicates "index not found".
    # Other errors should still be printed.
...
try:
    # Now, ensure the unique index is created
    movies_col.create_index("message_id", unique=True, background=True)
    print("Index 'message_id' (unique) ensured successfully.")
except DuplicateKeyError as e:
    # This specific error happens if there are duplicate message_ids in the collection
    # before a unique index can be created. This needs manual cleaning if it occurs.
    ...
    # General OperationFailure might still occur for other reasons
...
async def callback_handler(_, cq: CallbackQuery):
    ...
    if data == "confirm_delete_all_movies":
        ...
        # Ensure this block only handles the original admin feedback buttons (has, no, soon, wrong)
        # to avoid conflicts with the new noresult_ feedback.
