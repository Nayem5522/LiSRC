...
# ✅ UPDATED SEARCH FUNCTION WITH FUZZY SUGGESTIONS
...
async def search(_, msg):
    ...
    title_map = {clean_text(m["title"]): m for m in all_movies}
    close = get_close_matches(query, title_map.keys(), n=RESULTS_COUNT, cutoff=0.6)
    if close:
        await loading.delete()
        buttons = [
            [InlineKeyboardButton(title_map[c]["title"][:40], callback_data=f"movie_{title_map[c]['message_id']}")]
            for c in close
        ]
        m = await msg.reply("আপনার টাইপের সাথে মিলতে পারে এমন ফলাফল:", reply_markup=InlineKeyboardMarkup(buttons))
        asyncio.create_task(delete_message_later(m.chat.id, m.id))
        return
