...
async def set_filters(_, msg):
    btns = InlineKeyboardMarkup([
        [InlineKeyboardButton("Bengali", callback_data="lang_Bengali"), InlineKeyboardButton("Hindi", callback_data="lang_Hindi"), InlineKeyboardButton("English", callback_data="lang_English")],
        [InlineKeyboardButton("Clear Filters", callback_data="clear_filters")]
    ])
    await msg.reply("কোন ভাষার মুভি চান? নিচে সিলেক্ট করুন:", reply_markup=btns)

@app.on_callback_query(filters.regex("^lang_"))
async def language_filter(_, cq: CallbackQuery):
    lang = cq.data.split("_", 1)[1]
    users_col.update_one({"_id": cq.from_user.id}, {"$set": {"filter_language": lang}}, upsert=True)
    await cq.answer(f"{lang} ফিল্টার সেট হয়েছে!", show_alert=True)
    await cq.message.delete()

@app.on_callback_query(filters.regex("clear_filters"))
async def clear_filters(_, cq: CallbackQuery):
    users_col.update_one({"_id": cq.from_user.id}, {"$unset": {"filter_language": ""}})
    await cq.answer("সব ফিল্টার ক্লিয়ার হয়েছে!", show_alert=True)
    await cq.message.delete()
...
async def search(_, msg):
    ...
    user = users_col.find_one({"_id": msg.from_user.id}) or {}
    users_col.update_one({"_id": msg.from_user.id}, {"$set": {"last_search": datetime.utcnow()}}, upsert=True)

    filters_query = {}
    if user.get("filter_language"):
        filters_query["language"] = user["filter_language"]

    all_movies = list(movies_col.find(filters_query, {"title": 1, "message_id": 1}))
    exact_match = []
    suggestions = []
